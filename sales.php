...
function getSalesData($conn, $filter) {
    ...
    switch ($filter) {
        case 'daily':
            $sql = "SELECT p.name as product_name, SUM(s.quantity_sold) as quantity_sold, SUM(s.total_price) as total_price
                    FROM sales s
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($data->action === 'edit') {
        ...
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ii", $current_sales['quantity_sold'], $current_sales['product_id']);
        $stmt->execute();
